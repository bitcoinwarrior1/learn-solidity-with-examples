...
library BtcParser {
    ...
    function parseVarInt(bytes memory txBytes, uint pos) public returns (uint, uint) {
    ...
    function getBytesLE(bytes memory data, uint pos, uint bits) public returns (uint) {
        if (bits == 8) {
    ...
    function getFirstTwoOutputs(bytes memory txBytes) public
    ...
    function checkValueSent(bytes memory txBytes, bytes20 btcAddress, uint value) public
    returns (bool)
    {
    ...
    function scanInputs(bytes memory txBytes, uint pos, uint stop) public
    ...
    // takes a 'stop' argument which sets the maximum number of
    // outputs to scan through. stop=0 => scan all.
    function scanOutputs(bytes memory txBytes, uint pos, uint stop) public
    ...
    function sliceBytes20(bytes memory data, uint start) public returns (bytes20) {
        uint160 slice = 0;
    ...
    function isP2PKH(bytes memory txBytes, uint pos, uint script_len) public returns (bool) {
    ...
    function isP2SH(bytes memory txBytes, uint pos, uint script_len) public returns (bool) {
    ...
    function parseOutputScript(bytes memory txBytes, uint pos, uint script_len) public
    returns (bytes20)
    {
    ...
    function getPubKeyFromTx(bytes memory txBytes) public returns(bytes memory)
    {
        uint pos = 0;
        bytes memory pubkey;
    ...
    function getSegtwitSignature(bytes memory txBytes) public returns(bytes memory)
    {
        uint pos = 0;
